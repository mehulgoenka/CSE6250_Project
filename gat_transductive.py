...
"""
Graph Attention Network (GAT) implementation for transductive learning tasks on citation networks.
This script supports the Cora, Citeseer, and PubMed datasets.
...
def download_and_extract_dataset(dataset_name):
    ...
    content_path = os.path.join(dataset_dir, config['content_file'])
    cites_path = os.path.join(dataset_dir, config['cites_file'])
...
def load_data(dataset_name='cora'):
    ...
    if not os.path.exists(content_path) or not os.path.exists(cites_path):
        ...
        for root, dirs, files in os.walk(dataset_dir):
            if config['content_file'] in files and config['cites_file'] in files:
                content_path = os.path.join(root, config['content_file'])
                cites_path = os.path.join(root, config['cites_file'])
                break
        else:
            ...
            for line in lines[2:]:
                ...
                for token in tokens[2:]:
                    if token.startswith('feat'):
                        feature_values.append(float(token.split('=')[1]))
                idx_features_labels.append([paper_id] + feature_values + [label])
    ...
    if dataset_name == 'citeseer':
        # Citeseer: 20 nodes per class for training
        idx_train = []
        idx_val = []
        idx_test = []
        for i in range(labels.max().item() + 1):
            idx = (labels == i).nonzero(as_tuple=True)[0]
            idx_train.extend(idx[:20].tolist())
            idx_val.extend(idx[20:70].tolist())
            idx_test.extend(idx[70:].tolist())
        idx_train = torch.LongTensor(idx_train).to(device)
        idx_val = torch.LongTensor(idx_val).to(device)
        idx_test = torch.LongTensor(idx_test).to(device)
    elif dataset_name == 'pubmed':
        # PubMed: 60 nodes per class for training
        idx_train = []
        idx_val = []
        idx_test = []
        for i in range(labels.max().item() + 1):
            idx = (labels == i).nonzero(as_tuple=True)[0]
            idx_train.extend(idx[:60].tolist())
            idx_val.extend(idx[60:560].tolist())
            idx_test.extend(idx[560:].tolist())
        idx_train = torch.LongTensor(idx_train).to(device)
        idx_val = torch.LongTensor(idx_val).to(device)
        idx_test = torch.LongTensor(idx_test).to(device)
    else:
        # Cora: 20 nodes per class for training
        idx_train = []
        idx_val = []
        idx_test = []
        for i in range(labels.max().item() + 1):
